# Voice & Style

How The Weekly Thing sounds. This is the **issue voice** — Jamie writing the
intro, journal, and link commentary. [Echoes](echoes.md) uses a distinct
archive-librarian voice, but it is still part of the newsletter issue.

## Voice

- First-person, observational, dry, curious. **Not salesy.**
- Short sentences mixed with longer reflective ones. Comfortable with asides.
- Writes as a reader and a practitioner, not a pundit. No "thought leader" framing.
- Candid opinions, rarely hot-takes. Notices what others skim past.
- Zooms from a Wikipedia rabbit hole to a tech trend to a personal anecdote in one issue — **range
  is the feature.**

## What makes it distinctive (observable, not aspirational)

- Continuous since May 13, 2017. Three channels, one voice.
- **Commentary and Discovery:** every link has a reason it's in the issue. We learn together.
- Privacy-preserving by design: no click tracking. Specifically avoiding audience capture.
- Free for everyone; supporting membership has historically been pure charity
  with contributions directed to a non-profit.
- Place + family ground issues in a specific lived life.

## Guardrails (these override everything)

- No superlatives ("best", "amazing", "premier") or empty marketing phrases ("cutting-edge",
  "curated with care", "hand-picked").
- No "thought leader" / "bleeding edge" language; no ad-copy imperatives ("Level up your
  reading!") — prefer declarative framing.
- Use commentary-over-algorithm, 8+ years sustained, specific range, human editorial judgment.
- No emoji in generated copy unless quoting an issue subject that contains one.
- Prefer specific over general: "47 links about AI agents in the last year" beats "stay on top of
  AI."

## Voice markers worth keeping

Sincere use of **bold** and exclamation points; open acknowledgement of reversals and failed
experiments; specific numbers even when approximate; links that reference each other inside one
issue.

## Formatting

Per-section formatting rules (headings, link shapes, blank-line counts) live in
[`sections.md`](sections.md). Email rendering uses system-font fallbacks (Charter, Iowan Old Style,
SF Mono) because most clients don't load remote fonts — see
[`../reference/systems/buttondown.md`](../reference/systems/buttondown.md).
