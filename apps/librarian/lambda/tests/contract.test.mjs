...
const artifact = JSON.parse(
  await readFile(new URL('../../contracts/librarian-api.v1.json', import.meta.url), 'utf8')
);
...
test('contract negotiation remains additive for existing clients', () => {
  ...
  assert.equal(
    contractModule.supportsRequestedContract({ 'X-Librarian-Contract-Version': artifact.version }),
    true
  );
