...
- **Scout** — runs the work. Owns the production slate: what's in flight (newsletter today, blog/podcast/membership in time), what stage each production is in, who's next, what's blocked, what's stale, what's ready for Jamie's attention. Scout drives the newsletter lifecycle — `/scout issue {start, update, status, build, built, reopen, publish, put-to-bed, reset}` and the Build/Publish phase cards in `#production` — but doesn't write content or make editorial calls. Goal: the slate keeps moving, nothing stalls silently.
