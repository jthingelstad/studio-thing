...
class TriggerParsingTests(unittest.TestCase):
    def test_when_date_becomes_evening(self):
        self.assertEqual(
            follow_up.resolve_trigger(when="2026-06-01"), ("time", "2026-06-01T18:00:00", None)
        )

    def test_when_datetime_kept(self):
        self.assertEqual(
            follow_up.resolve_trigger(when="2026-05-13T18:30"),
            ("time", "2026-05-13T18:30:00", None),
        )
        self.assertEqual(
            follow_up.resolve_trigger(when="2026-05-13 18:30"),
            ("time", "2026-05-13T18:30:00", None),
        )
        # tz-aware → naive (local) form
        self.assertEqual(
            follow_up.resolve_trigger(when="2026-05-13T18:30:00-05:00"),
            ("time", "2026-05-13T18:30:00", None),
        )
    ...
    def test_in_days_offset(self):
        ...
        self.assertEqual(
            due,
            datetime.combine(
                date.today() + timedelta(days=1), datetime.min.time().replace(hour=18)
...
class DbHelperTests(_DBCase):
    def test_insert_open_due_fire_cancel(self):
        ...
        c = db.insert_follow_up(
            persona="eddy", trigger_kind="issue", note="at 387", trigger_issue=387
        )
    ...
    def test_create_helper(self):
        row = follow_up.create(
            persona="eddy", note="check the issue", in_days=7, created_by="jamie"
        )
...
class SweepTests(_DBCase):
    def setUp(self):
        ...
        self._p = patch.object(
            follow_up, "_persona_context_block", lambda persona: "_(test context)_"
        )
        self._p.start()
    ...
    def test_fires_due_posts_and_leaves_others(self):
        past = (datetime.now() - timedelta(hours=1)).isoformat(timespec="seconds")
        future = (datetime.now() + timedelta(days=5)).isoformat(timespec="seconds")
        a = db.insert_follow_up(
            persona="eddy", trigger_kind="time", note="how WT348 is shaping up", due_at=past
        )
    ...
    def test_issue_trigger_fires_when_window_reaches(self):
        ...
        fid = db.insert_follow_up(
            persona="eddy", trigger_kind="issue", note="revisit when we hit 387", trigger_issue=387
        )
        ft = _FakeTeam(persona="eddy")
        res = asyncio.run(follow_up.sweep(ft.ctx()))
...
class FollowupToolTests(_DBCase):
    ...
    def test_schedule_list_cancel(self):
        out = self._as(
            "eddy", agent_tools.t_followup_schedule, note="check WT350 framing", at_issue=350
        )
        ...
        self.assertEqual(
            self._as("eddy", agent_tools.t_followup_cancel, followup_id=fid)["cancelled"], True
        )
...
class WiringTests(unittest.TestCase):
    ...
    def test_followup_subgroup_wired(self):
        ...
        self.assertEqual(
            {getattr(c, "_cmd_name", None) for c in fu.commands}, {"list", "add", "cancel"}
        )
...
class FollowUpSweepLockTests(_DBCase):
    ...
    def test_concurrent_sweep_is_blocked_by_job_lock(self):
        ...
        past = (datetime.now() - timedelta(hours=1)).isoformat(timespec="seconds")
        db.insert_follow_up(persona="eddy", trigger_kind="time", note="x", due_at=past)
        ft = _FakeTeam(persona="eddy")
