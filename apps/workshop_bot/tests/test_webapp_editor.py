...
class WebappEditorTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        db.run_migrations()
        db.create_production(production_type="newsletter", title="WT349", seq=349, phase="build")
    ...
    async def test_atom_save_issue_item_body_override(self):
        ...
        self.assertEqual(r.status, 302)
        self.assertEqual(r.headers["Location"], f"/productions/WT349#atom-item-{self.n1}")
    ...
    async def test_flip_select_move_roundtrip(self):
        ...
        r = await c.post(
            "/productions/WT349/editor/select",
            headers=H,
            allow_redirects=False,
